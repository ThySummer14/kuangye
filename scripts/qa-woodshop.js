async (page) => {
  ...
  for (const width of [1280, 375]) {
    ...
    await page
      .getByRole("button", { name: "雨后蓝 选这块墙色", exact: true })
      .click();
    await page
      .getByRole("button", { name: "暖胡桃木 铺上这款木地板", exact: true })
      .click();
    await page
      .getByRole("button", { name: "请师傅开工 · 30 光", exact: true })
      .click();
    await page.getByRole("button", { name: "再想想", exact: true }).click();
    let s = await page.evaluate(() => window.__KUANGYE__.snapshot());
    if (
      s.home.lumens !== 500 ||
      s.home.decor.wall !== "blue" ||
      s.home.decor.floor !== "walnut"
    )
      throw Error("Cancel/decor");
    await page
      .getByRole("button", { name: "请师傅开工 · 30 光", exact: true })
      .click();
    ...
    await page.screenshot({
      path: "output/playwright/woodshop-" + width + ".png",
      fullPage: true,
    });
    results.push(s);
    for (const axis of ["向右扩建 →", "向前扩建 ↓"]) {
      ...
      while (await page.getByRole("button", { name: /请师傅开工/ }).count()) {
        ...
        await page
          .getByRole("button", { name: "确认扩建", exact: true })
          .click();
      }
    }
    s = await page.evaluate(() => window.__KUANGYE__.snapshot());
    if (s.home.lumens !== 20 || s.home.room.w !== 12 || s.home.room.d !== 12)
      throw Error("Cap/economy");
    results.push(s);
    ...
    if (
      (await page.evaluate(() => window.__KUANGYE__.snapshot())).viewport
        .overflow
    )
    ...
    if (await page.getByRole("button", { name: "找师傅聊聊装修" }).count())
      throw Error("Craftsman still at home");
  }
  return results;
};
