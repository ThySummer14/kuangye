...
export function registerGameTools(
  navigate,
  getPlace = () => location.hash.slice(1) || "map",
) {
  const snapshot = () =>
    JSON.parse(
      JSON.stringify({
  ...
  window.__KUANGYE__ = Object.freeze({
    snapshot,
    ...(QA
      ? {
          ...
          async reset(name, variant = "funded") {
            ...
            if (variant === "furnished") {
              ...
              home.inventory = fixtures.map(([fid], i) => ({
                uid: `qa-${i}`,
                fid,
                paid: 0,
              }));
              home.placed = fixtures.map(([, x, z], i) => ({
                uid: `qa-${i}`,
                x,
                z,
                rotation: 0,
            ...
            home.decor.light = "day";
            home.decor.weather = "clear";
            Object.assign(state, {
              home,
              active: [],
              done: [],
