...
function normalizeState(raw) {
  ...
  return {
          ...
          .map((d) => ({
            ...
            units: Array.isArray(d.units)
              ? d.units
                  ...
                  .map((u) => ({ metric: u.metric, v: Math.max(0, cleanNumber(u.v)) }))
              : [],
            logs: Array.isArray(d.logs)
              ? d.logs
                  .filter((l) => l && typeof l.d === 'string')
                  .map((l) => ({ d: l.d, ...(l.shield ? { shield: true } : {}) }))
              : [],
            ...(d.streak !== undefined ? { streak: Math.max(0, Math.floor(cleanNumber(d.streak))) } : {}),
          }))
      : [],
    abandoned: Array.isArray(source.abandoned)
      ...
      : [],
    settings: { devDate: typeof source.settings?.devDate === 'string' ? source.settings.devDate : '' },
  }
}
...
watch(
  state,
  () => {
    try { localStorage.setItem(KEY, JSON.stringify(state)) } catch (e) { /* 存储空间不足时仍允许当前页继续使用 */ }
  },
  { deep: true }
)
...
export function now() {
  ...
  return /^\d{4}-\d{2}-\d{2}$/.test(devDate || '') ? new Date(devDate + 'T12:00:00') : new Date()
}
...
export function progressOf(a) {
  ...
  if (t.type === 'total') return { cur: a.logs.reduce((s, l) => s + Math.max(0, Number(l.v) || 0), 0), target: t.target }
...
export function canUseShield(a) {
  return taskById[a.qid]?.type === 'streak' && a.start <= yesterday() && a.shields > 0 && !a.logs.some((l) => l.d === yesterday())
}
...
export function complete(a, review = '') {
  const t = taskById[a.qid]
  if (!state.active.includes(a) || !t || (t.type !== 'once' && !reached(a))) return false
  const p = progressOf(a)
  const units = []
  ...
  state.done.push({ qid: t.id, xp: DIFF[t.diff].xp, at: today(), review, units, logs: a.logs.map((l) => ({ ...l })), ...(t.type === 'streak' ? { streak: p.cur } : {}) })
  state.active = state.active.filter((x) => x !== a)
  buddyMoment('celebrate', 2000, '又变强了一点！')
  return true
}
export function abandon(a, reason = '') {
  if (!a || !taskById[a.qid]) return false
...
export function exportData() {
  return JSON.stringify(
    { app: 'kuangye', version: 2, exportedAt: new Date().toISOString(), state: JSON.parse(JSON.stringify(state)) },
    null,
    2
  )
}
